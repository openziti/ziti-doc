...
CMD="time npm install -g linkinator && time linkinator \"${SERVER}\"; exit \$?"
...
time docker run --rm -t --name linkinator -v "/tmp:/mount" -v "$PWD/linkinator.config.json":/linkinator.config.json node:22-slim bash -cx "$CMD"
