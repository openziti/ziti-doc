...
#   ./run.sh --accept-visual-changes            # skip comparison + pass (intentional redesign; CI label)
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --accept-visual-changes) ACCEPT_CHANGES=1; shift ;;
...
# Intentional visual change accepted (e.g. a PR label): report success without comparing.
# The post-merge publish job regenerates the baselines on main. Lets a deliberate redesign
# pass a required "verify" check that would otherwise block on the expected diff.
if [[ "$ACCEPT_CHANGES" == "1" ]]; then
  echo ">> --accept-visual-changes set: skipping visual comparison (baselines will be"
  echo ">> regenerated by the publish job after merge to main)."
  exit 0
fi
