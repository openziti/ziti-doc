- `enrollToCertEnabled` - Allows clients with valid JWTs to request a certificate from the controller by providing a
  [Certificate Signing Request (CSR)](https://en.wikipedia.org/wiki/Certificate_signing_request). The resulting
  certificate will be used for authentication. The target `enrollAuthPolicyId` must be set to a policy that allows
  certificate authentication. If `enrollAuthPolicyId` is not set, the default policy
...
- `enrollAuthPolicyId` - the id of the [Authentication Policy](authentication/50-authentication-policies.md) to use
  for the enrolling identity. Defaults to no value set and the identity will enroll with the default Authentication
  Policy.
