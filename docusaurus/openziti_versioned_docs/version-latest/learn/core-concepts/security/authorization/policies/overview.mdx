...
# Policies
OpenZiti policies control which [Identities](../../authentication/80-identities.md) can access or host which services
...
particular services or service groups.
  ...
  * Geographic/political boundaries. Some services may need to be accessible only from specific areas to comply with
    local laws
...
in the policy. All OpenZiti policies work the same way. 
  ...
  * The entities included in policies (Identities, services and edge routers) can all be assigned role attributes
      * For example: an Identity could be given the attributes `sales`, `eu` if that Identity belongs to a sales
        person from the European Union
  ...
  * Each list can contain role attributes, which are prefixed by hash tags, and ids, which are prefixed with at-symbols.
      ...
      * Note that external tools such as the CLI and the ZAC will likely use names instead of IDs. So the roles might
        instead look like
  ...
  * Each policy also has a semantic. The semantic dictates how multiple role attributes will be interpreted.
    `[#sales, #eu]` could mean every Identity which has `#sales` *and* `#eu`, or it could mean every Identity which
    has `#sales` *or* `#eu`
  ...
  * There is a special role attribute `#all` which will include all entities of a given type. This is useful for
    simpler setups which may not be segmenting edge routers, as well as for development and testing.
...
When an Identity is trying to establish an [API Session](../../sessions.md#api-session) to use or host a service,
the OpenZiti controller will verify that it has been granted access to the service via Service Policy. Once the
[API Session](../../sessions.md#api-session) is established, the controller will return a list of edge routers that
the Identity
