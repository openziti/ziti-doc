...
const REPOS = {
    ziti: { data: ziti, label: 'openziti / ziti', color: COLORS.ziti },
    zrok: { data: zrok, label: 'openziti / zrok', color: COLORS.zrok },
    others: { data: others, label: 'openziti / others', color: COLORS.others },
};
/* ------------------ Date Utilities ------------------ */
const dateUtils = {
    stripTime: (d) => new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate())),
    byDate: (a, b) => new Date(a.date).getTime() - new Date(b.date).getTime(),
    parseLocalToUtc: (val) => {
        if (!val)
            return NaN;
    ...
    toInputValue: (ms) => new Date(ms).toISOString().slice(0, 16),
    formatDisplay: (ms) => new Date(ms).toISOString().slice(0, 10),
    tickFormat: (ts, span) => {
        const d = new Date(ts);
        if (span > 2 * YEAR) {
...
/* ------------------ Data Processing ------------------ */
function processStarData(events) {
    ...
    const daily = Array.from(dailyMap.entries()).sort((a, b) => a[0] - b[0]);
    // Cumulative counts
    const cumulative = [];
    let count = 0;
...
function calculateStats(events, range) {
    if (!events.length)
        return null;
    const times = events
...
function createSeriesPair(name, color, daily, cumulative) {
    return [
        {
...
function Controls({ snapMidnight, setSnapMidnight, startDate, setStartDate, endDate, setEndDate, setIsDateEditing }) {
    return (_jsxs("div", { style: {
            display: 'flex', alignItems: 'center', gap: '12px',
            margin: '4px 0 12px', fontSize: '0.9rem'
        }, children: [_jsxs("label", { style: { display: 'flex', alignItems: 'center', gap: 4 }, children: [_jsx("input", { type: "checkbox", style: { width: 14, height: 14 }, checked: snapMidnight, onChange: e => setSnapMidnight(e.target.checked) }), "Snap to midnight UTC"] }), _jsxs("label", { children: ["Start:\u00A0", _jsx("input", { type: "datetime-local", value: startDate, onFocus: () => setIsDateEditing(true), onBlur: () => setIsDateEditing(false), onChange: e => setStartDate(e.target.value) })] }), _jsxs("label", { children: ["End:\u00A0", _jsx("input", { type: "datetime-local", value: endDate, onFocus: () => setIsDateEditing(true), onBlur: () => setIsDateEditing(false), onChange: e => setEndDate(e.target.value) })] })] }));
}
function StatsTable({ rows, range }) {
    return (_jsxs("div", { style: {
            margin: '8px 0 12px',
            border: '1px solid var(--ifm-table-border-color,#e3e3e3)',
            borderRadius: 8, padding: 12
        }, children: [range && (_jsxs("div", { style: { marginBottom: 8, fontWeight: 600 }, children: ["Date range: ", dateUtils.formatDisplay(range[0]), " \u2192 ", dateUtils.formatDisplay(range[1])] })), _jsxs("table", { style: { width: '100%', borderCollapse: 'separate', borderSpacing: 0 }, children: [_jsx("thead", { children: _jsxs("tr", { children: [_jsx("th", { style: { textAlign: 'left' }, children: "Repo" }), _jsx("th", { children: "Stars" }), _jsx("th", { children: "Avg/day" }), _jsx("th", { children: "Days w/ stars" }), _jsx("th", { children: "Max/day" }), _jsx("th", { children: "Peak day" })] }) }), _jsx("tbody", { children: rows.map(({ label, color, stats }) => (_jsxs("tr", { children: [_jsx("td", { children: _jsx("span", { style: {
                                            background: color, color: '#fff',
                                            padding: '2px 8px', borderRadius: 999
                                        }, children: label }) }), _jsx("td", { style: { textAlign: 'center' }, children: stats?.stars ?? 0 }), _jsx("td", { style: { textAlign: 'center' }, children: stats?.avgPerDay ?? 0 }), _jsx("td", { style: { textAlign: 'center' }, children: stats?.daysWithStars ?? 0 }), _jsx("td", { style: { textAlign: 'center' }, children: stats?.maxInOneDay ?? 0 }), _jsx("td", { style: { textAlign: 'center' }, children: stats?.dayWithMost ?? '—' })] }, label))) })] })] }));
}
/* ------------------ Main Component ------------------ */
export default function Stargazers() {
    ...
    const [liveSpan, setLiveSpan] = useState(0);
    ...
    const chartOption = useMemo(() => {
        ...
        const xAxisConfig = {
            ...
            axisLabel: {
                ...
                showMinLabel: true,
                showMaxLabel: true,
                formatter: (val) => dateUtils.tickFormat(val, span)
            },
    ...
    return (_jsxs(NetFoundryLayout, { className: styles.landing, starProps: starProps, footerProps: openZitiFooter, children: [_jsx(Head, { children: _jsx("meta", { "data-rh": "true", name: "nf-pages-version", content: "NFLayoutVersion" }) }), _jsx(ReactEcharts, { option: chartOption, style: { width: '100%', height: CHART_HEIGHT }, onEvents: onEvents, onChartReady: handleReady }), _jsx(Controls, { snapMidnight: snapMidnight, setSnapMidnight: setSnapMidnight, startDate: startDate, setStartDate: setStartDate, endDate: endDate, setEndDate: setEndDate, setIsDateEditing: setIsDateEditing }), _jsx(StatsTable, { rows: statsRows, range: range })] }));
}
