...
// USE_DOCS_FOR_BASE_URL toggles between two supported deployment modes:
//   unset/false: site lives at host root, docs at /docs/openziti/... (production today)
//   true:        site is hosted under /docs/, docs at /docs/openziti/... via baseUrl
// Final URL paths are identical in both modes; only the split between baseUrl and
// docsBase differs. No other values are supported -- arbitrary base paths would
// require fixing content-level absolute /docs/... links across the site.
import {useDocsForBaseUrl, baseUrl, openziti, docsBase} from './src/config-paths';
...
const config: Config = {
    ...
    // Production builds (baseUrl='/') are strict. SPA-hosted builds (baseUrl='/docs/')
    // demote to 'warn' because some routes generated by sub-plugins are reported as
    // broken under the alternate baseUrl even when the rendered URLs are correct at
    // runtime. Real content errors are still caught by production CI.
    onBrokenLinks: useDocsForBaseUrl ? 'warn' : 'throw',
    ...
    customFields: {
        DOCUSAURUS_BASE_PATH: useDocsForBaseUrl ? '' : '/docs',
    ...
    plugins: [
        ...
        // The third arg is the docs route base path RELATIVE TO baseUrl. In production
        // (baseUrl='/') we want pages at /docs/<openziti>. In SPA mode (baseUrl='/docs/')
        // we want pages at the same canonical /docs/<openziti>, so the arg drops the
        // /docs/ prefix that baseUrl already provides.
        openzitiDocsPluginConfig(__dirname, REMARK_MAPPINGS, useDocsForBaseUrl ? openziti : `docs/${openziti}`),
    ],
    presets: [
        [ // Redocusaurus config
            'redocusaurus',
