...
The tunneler CLI is installed automatically with the Ziti Desktop Edge for Mac. The tunneler CLI can be used to manage MFA separately from Ziti Desktop Edge for Mac.

Using Terminal.app, complete the following workflow to register a time-based one-time password (TOTP) token generator with the OpenZiti Controller.
