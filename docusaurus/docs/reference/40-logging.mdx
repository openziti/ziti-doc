...
This article is about log messages from the `ziti` CLI that is used to run the controller and router. See [the Linux tunneler reference](/how-to-guides/tunnelers/60-linux/50-linux-tunnel-troubleshooting.mdx) for details about messages from `ziti-edge-tunnel`.
