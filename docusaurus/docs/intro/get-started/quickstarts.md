...
OpenZiti works with your applications at whatever level makes sense. Use lightweight [tunnelers](@openzitidocs/how-to-guides/tunnelers/index) to add zero trust to existing applications with no code changes, or embed our [SDKs](@openzitidocs/reference/developer/sdk/index) directly for the strongest security posture. Either way, your services stay dark: no open ports, no public endpoints, no exposure to unauthorized users.
...
[network](@openzitidocs/intro/what-is-openziti/index). For someone just starting out, there are four basic options:
