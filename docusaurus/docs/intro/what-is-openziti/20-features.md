...
Traditional network security is exposed to the internet with at least some open ports allowing attackers to scan and potentially find an exploit. OpenZiti removes this threat by making services dark - no listening ports, no public endpoints. You can use tunnelers to add zero trust to existing applications with no code changes, but you can also embed an SDK directly so the application holds its own cryptographic identity without trusting the host OS network.
...
Deploying software to desktop/server is never an easy task. Applications powered by OpenZiti enable you to reduce the deployment time & effort. Use lightweight [tunnelers](@openzitidocs/how-to-guides/tunnelers/index) to add zero trust to existing applications with no code changes - available for a variety of operating systems, including mobile. For the strongest security posture, integrate an [SDK](@openzitidocs/reference/developer/sdk/index) directly into your application so it holds its own cryptographic identity.
