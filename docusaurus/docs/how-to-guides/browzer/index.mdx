...
of enabling BrowZer, see the [Example Enabling BrowZer](@openzitidocs/how-to-guides/browzer/example) subpage showing the steps in more detail.
...
* you have signed up with Chrome's "origin trials" to allow the BrowZer bootstrapper to automatically enable
  [JSPI](https://v8.dev/blog/jspi) for your BrowZer users. See [the chrome-origin-trials](@openzitidocs/how-to-guides/browzer/chrome-origin-trials)
  for more detail 
...
* create an [external jwt signer](@openzitidocs/learn/core-concepts/security/authentication/70-external-jwt-signers)
* create an [authentication policy](@openzitidocs/learn/core-concepts/security/authentication/50-authentication-policies)
...
If you have used the [Local - No Docker](@openzitidocs/intro/get-started/network/local-no-docker) or the
["host it anywhere" quickstart](@openzitidocs/intro/get-started/network/hosted) quickstarts, you would have sourced the 
