...
* an identity exists with an `external-id` properly mapped to a claim, 
  see [External Id & x509 Claims](../../../../learn/core-concepts/security/authentication/30-third-party-cas.md#external-id-and-x509-claims)
* The CA's JWT has been transferred to the computer running the ZDEW

<Details>
<summary><b>Obtaining the 3rd-party CA JWT</b></summary>

Adding an identity to a Windows machine that uses a 3rd-party CA requires the user or an operator to obtain a JWT
ahead of time. This can be done in two different ways.
<hr/>

### Obtain the 3rd-party CA JWT - ZAC

Obtain a 3rd-party CA's JWT using the Ziti Admin Console. From the Authentication->Certificate Authorities page, 
click the icon in the JWT column for the appropriate CA and send the JWT to the user trying to add an
identity.
...
With the JWT for the CA on the machine running the ZDEW, click the "ADD IDENTITY" button in the top right of the
screen. After the context menu pops up choose "With JWT". In the file dialog, select the 3rd-party CA JWT file.
...
![3rd-party CA](/img/ext-jwt-signer/windows-3rd-party-ca.png)
