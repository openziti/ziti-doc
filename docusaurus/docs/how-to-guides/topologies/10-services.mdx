---
sidebar_position: 50
sidebar_label: Service topologies
title: Service topologies
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';

These examples illustrate common service topology patterns in OpenZiti deployments.

## 3.0 Ziti services

Ziti services take several forms in deployment environments. In this guide, we will highlight a few examples.
...
## 3.2 Set up VMs for ziti network

### 3.2.1 Controller

#### 3.2.1.2 CLI

You will need to log in to the controller to provision identities and service. Please make sure you are performing the action on the right node.

On the controller, before performing the CLI command, you will need to log in first:
```text
zitiLogin
```
...
- Public edge router (**pub-er**) was setup in the [Router setup section](../deployments/40-cloud/20-router.mdx#23-create-and-set-up-router-directly-on-router-vm), this router provides fabric and edge connection. It does not have tunneler functionality. Make sure you also modify the [Firewall](/how-to-guides/deployments/40-cloud/20-router.mdx#29-firewall) for this router.
- Local edge router (**local-er**). Please follow the [Router guide](../deployments/40-cloud/20-router.mdx) to set up a [Router with edge listener and tunneler](../deployments/40-cloud/20-router.mdx#2343-create-the-router-with-edge-listener-and-tunneler). For this router, you will need to set up [Resolver](/how-to-guides/deployments/40-cloud/20-router.mdx#26-fix-the-resolver), [Route Table](/how-to-guides/deployments/40-cloud/20-router.mdx#27-route-table), [Source and Destination Check](/how-to-guides/deployments/40-cloud/20-router.mdx#28-source-and-destination-check) and [Firewall](/how-to-guides/deployments/40-cloud/20-router.mdx#29-firewall).
...
We need two tunnelers for our testing. Please follow **[Create a VM section](/how-to-guides/deployments/40-cloud/10-controller.mdx#11-create-a-vm-to-be-used-as-the-controller)** of the Controller Guide to create two VMs running Ubuntu 22.04.
...
```text
ziti edge create identity ingress-tunnel -o ingress-tunnel.jwt
ziti edge create identity egress-tunnel -o egress-tunnel.jwt
```

#### 3.2.3.3 Register identities

Log in to your VMs created earlier, follow [Install Linux Package](../../how-to-guides/tunnelers/60-linux/index.mdx) **Ubuntu Jammy 22.04** section to install and register your ziti-edge-tunnel.
...
To add attribute "public" to the router, choose your router from the **ROUTERS** screen, click **...** and choose **Edit**
...
## 3.4 Set up tunneled SSH connection
...
This config instructs the server-side tunneler how to offload the traffic from the overlay, back to the underlay. We are dropping the traffic off our loopback interface, so we use the address "127.0.0.1“
...
Log in to the intercept side tunneler (**ingress-tunnel**) node, you should be able to ssh to the server (**egress-tunnel**) by using dns name "t2tssh.ziti"
...
Routers have an optional tunneler function. In this section, we will demonstrate a connection between a tunneler and such a router with an enabled tunneler.

The procedure is similar to tunnel to tunnel service, so please refer to that section for a detailed explanation of each step.
...
Ensure both policies are set up correctly.
...
Ensure policy advisor reports "OKAY."
...
Log in to the router (**local-er**), set up a file to send via http.
...
Log in to the intercept side tunneler (**egress-tunnel**) node.
...
In this section, we will demonstrate how to set up non-OpenZiti client to use the ziti service. Please refer to [Network diagram 2](#312-network-diagram-2) for our test setup.
...
Log in to the client (**egress-tunnel**), set up a file to send via http.
...
**set up the route first**. The route is via our ER in the same DC (10.124.0.2)
...
**set up the route first**. The route is via our ER in the same DC (10.162.209.220)
...
- We need to set up the route for 100.64/10 subnet
...
In this section, we will demonstrate how to set up OpenZiti service to access a server from a SDK enabled application. Please refer to [Network diagram 3](#313-network-diagram-3) for our test setup.
...
Log in to the client (**egress-tunnel**), make sure "hello.txt" is still there from previous step. If not, [recreate it](#367-verify-the-connection).
...
In this section, we will demonstrate how to set up OpenZiti service to access a SDK enabled application via ziti-tunnel. Please refer to [Network diagram 4](#314-network-diagram-4) for our test setup.
...
Since the server is an SDK application, there is no need for `host.v1` config. 
...
Log in to the "[sdk-application-server](#382-sdk-application-server)" node, make sure you are under the "simple-server" directory, start the server by using the registered server identity and service name.
...
Log in to the client (**ingress-tunnel**), try the service with the service name we create and any text in the "name" field.
