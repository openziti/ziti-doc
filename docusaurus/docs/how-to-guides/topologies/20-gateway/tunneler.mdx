---
sidebar_position: 20
sidebar_label: Tunneler as a local gateway
title: Use a tunneler as a local gateway
---

## 1.0 Introduction

Run `ziti-edge-tunnel run` to set up an infrastructure gateway providing an IP route and nameserver for Ziti services on a local subnet.
...
- An open-ziti network should be created already. If not, please follow this quickstart [Host OpenZiti Anywhere](../../../get-started/network/hosted.mdx) guide to set up a ziti network first.
...
## 2.0 Set up tunnelers

## 2.1 Create tunnelers on the controller

Log in to your controller and create two identities named **local-tunnel** and **remote-tunnel**
...
## 2.2 Set up the tunneler for Windows subnet

### 2.2.1 Register identities

Log in to your **local-tunnel** machine/VM. Follow [Install Linux Package:](../../../how-to-guides/tunnelers/60-linux/index.mdx) **Ubuntu Jammy 22.04** section to install and register your `ziti-edge-tunnel`. Use the **local-tunnel.jwt** created earlier for its identity token.
...
## 2.3 Set up the tunneler for Ubuntu server subnet

### 2.3.1 Register identities

Log in to your **remote-tunnel** machine/VM. Register the tunneler by using **remote-tunnel.jwt** token with the instructions mentioned in [this section](#221-register-identities).
