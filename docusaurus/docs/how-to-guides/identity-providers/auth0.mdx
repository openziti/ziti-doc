...
<img src="@static/icons/logo-auth0.svg" alt="Auth0 logo" height="100px"/>
...
From the left navbar in the Auth0 dashboard, click `Application` -> `APIs` -> `Create API`
...
after successful authentication. Choose RFC 9068 as the JSON Web Token Profile, then click **Create**.
...
Auth0 opens the new API after it is created. Select the **Application Access** tab, find the application created above
and click **Edit**.

:::info[Application Access]
![Auth0 application access](/img/idps/auth0/application-access.png)
:::

On the **User-Delegated Access** tab click **Grant Access**. Without this grant Auth0 will not issue tokens for the
API to the application.

:::info[Grant Access]
![Auth0 grant access](/img/idps/auth0/grant-access.png)
:::
