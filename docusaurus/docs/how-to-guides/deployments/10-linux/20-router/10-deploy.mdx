...
Deploy a router as a Linux system service. [The router introduction](@openzitidocs/intro/what-is-openziti/03-components#router) may help to read first.
...
Create the router in the controller first with [the web console](@openzitidocs/how-to-guides/deployments/10-linux/30-console) or [the CLI](@openzitidocs/how-to-guides/deployments/10-linux/20-router/40-cli-mgmt).

After [creating the router](@openzitidocs/how-to-guides/deployments/10-linux/20-router/40-cli-mgmt#create-a-router), save the enrollment token (JWT) for the configuration step below.
...
Create a config file directly with `ziti create config router edge --routerName=router`. Run `ziti create config environment` to see the available environment variables. See [the router configuration reference](@openzitidocs/reference/30-configuration/router) for details.
...
Here's a link to [the configuration reference](@openzitidocs/reference/30-configuration/router).

Learn more about [managing routers with the CLI](@openzitidocs/how-to-guides/deployments/10-linux/20-router/40-cli-mgmt).
...
Learn more in [the logging reference](@openzitidocs/reference/40-logging).
