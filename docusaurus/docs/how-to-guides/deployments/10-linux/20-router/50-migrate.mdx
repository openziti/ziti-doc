...
# Migrate the router installation

Here's an example of migrating an existing router configuration to the Linux service's working directory.

1. Remove the quickstart router service if you followed the Bash quickstart to create
   **/etc/systemd/system/ziti-router.service**.
1. Follow [the Linux router deployment guide](/how-to-guides/deployments/10-linux/20-router/10-deploy.mdx) to
...
1. Copy the router-related parts of the configuration to the service's working directory and rename the
   configuration file to `config.yml`.
