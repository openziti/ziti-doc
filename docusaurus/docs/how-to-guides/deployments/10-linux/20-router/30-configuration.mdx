...
This article expands on the [Linux router deployment](/how-to-guides/deployments/10-linux/20-router/10-deploy.mdx) article with configuration concepts and examples.
...
The private keys' values are assumed to be `file://` URLs if unspecified. Alternatives include `pkcs11://` and `parsec://` URLs for [hardware keys](/how-to-guides/hsm/index.md).
