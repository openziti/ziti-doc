...
This article expands on the [Linux router deployment](@openzitidocs/how-to-guides/deployments/10-linux/20-router/10-deploy) article with configuration concepts and examples.
...
The private keys' values are assumed to be `file://` URLs if unspecified. Alternatives include `pkcs11://` and `parsec://` URLs for [hardware keys](@openzitidocs/how-to-guides/hsm/index).

## Optional Capabilities

You may enable or disable capabilities of the router by including or omitting their sections in the configuration file. These are a few of the most relevant configuration sections. Refer to [the router configuration reference](@openzitidocs/reference/30-configuration/router) for more complete information.

- [`edge`](@openzitidocs/reference/30-configuration/router#edge) - Most routers include this section which configures the router to listen for connections from endpoint identities. This is what makes a router an "edge router." Without this section, the router is a "fabric router" and communicates only with other routers and the control plane.
- [`link.listeners`](@openzitidocs/reference/30-configuration/router#link) - The `link` section configures the router to dial and listen for other routers that are dialing in to create mesh links. Omitting the `listeners` sub-section makes a router "private." "Public" routers are presumed reachable and listening for other routers.
- [`listeners[?binding == 'tunnel']`](@openzitidocs/reference/30-configuration/router#listeners) - the `listeners` section may contain a special type of binding that configures the router's built-in tunneling capabilities: `binding: tunnel`. The tunnel's `mode` property may be set to `tproxy`, `proxy`, or `host`. Requires `edge`.
