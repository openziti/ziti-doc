...
1. Configure the controller
    ...
    The console's `zac` binding must be on the same web listener as the controller's management API, i.e., the
    `edge-management` binding.
    ...
    - `location` is the filesystem path to the static files of the console. In the controller container, the
      usual path is `/ziti-console`.
