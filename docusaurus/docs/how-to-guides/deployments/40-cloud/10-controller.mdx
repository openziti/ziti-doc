...
- Sign in to the Digital Ocean console and create a **Droplets** from the dropdown menu on the upper right-hand side.
...
- Log in to the VM with username "ubuntu":
...
- Log in to the VM with username "root" and IP address:
```text
ssh root@<ip>
```
...
- Log in to the VM with username "ubuntu" and the IP address:
```text
ssh -i <private_key> ubuntu@<ip>
```
