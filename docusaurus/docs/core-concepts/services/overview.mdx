...
* **Configuration** - Ziti allows application specific configuration to be stored for services. See [Configuration Store](../core-concepts/config-store/overview)
* **Authorization** - For a details on controlling access to services, see [Policies](../core-concepts/security/authorization/policies/overview).
