...
An alternative method for invoking the snapshot operation is via the IPC agent, which must be run on the same host by the same user as the running OpenZiti Controller.
