...
The router's generated configuration always uses a single TCP port (default: `3022`) defined by `ZITI_ROUTER_PORT` in `/opt/openziti/etc/router/bootstrap.env`.

Confirm the router is running and listening on the expected port.

```bash
systemctl show -p MainPID --value ziti-router.service \
| xargs -rIPID sudo lsof -Pnp PID \
|& grep LISTEN
```

```buttonless title="Output"
ziti    3574455 ziti-router    7u     IPv6           41543151       0t0      TCP *:3022 (LISTEN)
```

Substitute the router's advertised address and port for `localhost:3022` as defined in `/var/lib/ziti-router/config.yml`.

```text
EXTERNAL_URI=localhost:3022
```

Verify the router is reachable and presents a server certificate from the controller's intermediate edge signer CA for both ALPN identifiers: `ziti-edge` and `ziti-link`.

```text
for ALPN in ziti-{edge,link}; do
    openssl s_client -connect ${EXTERNAL_URI} -alpn $ALPN -showcerts <>/dev/null \
