...
1. `ZITI_ROUTER_ADVERTISED_ADDRESS` — permanent DNS name or IP address (required, default: localhost)
...
```text
sudoedit /opt/openziti/etc/router/override.conf.example
```

Uncomment the directives you need, then apply:

```text
sudo systemctl edit ziti-router.service --stdin < /opt/openziti/etc/router/override.conf.example
sudo systemctl restart ziti-router.service
```
...
```text
sudo systemctl edit ziti-router.service
```
