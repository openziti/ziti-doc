...
For greater certainty that the router service is listening on the expected port, this one-liner uses `systemctl` to get the router's PID, and `xargs` to pass the PID to `lsof` to list its open files, and `grep` to filter for listening sockets.
