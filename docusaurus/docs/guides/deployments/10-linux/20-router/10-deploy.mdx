...
- **Router address** — permanent DNS name or IP address (default: system hostname)
...
- **Answer file** — use **/opt/openziti/etc/router/bootstrap.env** as a reference template for the available variables
...
Answers are written to a temporary file during bootstrap and deleted automatically on success. If bootstrap fails, the temporary file is preserved in `/tmp/` for debugging.
...
1. `ZITI_ROUTER_ADVERTISED_ADDRESS` — permanent DNS name or IP address (default: system hostname)
1. `ZITI_ROUTER_PORT` — TCP port for routers (default: 3022)
...
The router learns the controller's address from the enrollment token, so you don't normally need to supply it. If you need to override the address in the enrollment token, supply these answers via environment variables or stdin:
