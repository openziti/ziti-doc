...
Or, manually configure the package repo and install `openziti-controller`.
...
1. Set the required values in the answer file **/opt/openziti/etc/controller/bootstrap.env**.
    1. `ZITI_CTRL_ADVERTISED_ADDRESS` - control plane permanent DNS name
    1. `ZITI_CTRL_ADVERTISED_PORT` - listener TCP port (default: 1280)
    1. `ZITI_CTRL_BIND_ADDRESS` - listener interface (default: 0.0.0.0)
    1. `ZITI_USER` - username (default: admin)
    1. `ZITI_PWD` - password to initialize database
1. Run `/opt/openziti/etc/controller/bootstrap.bash`
