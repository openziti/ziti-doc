...
title: Use a Tunneler as a Gateway
---

## 1.0 Introduction

This article is about using `ziti-edge-tunnel run` as an infrastructure gateway providing access to services for a local subnet.
...
- An open-ziti network should be created already. If not, please follow this quickstart [Host OpenZiti Anywhere](/learn/quickstarts/network/hosted.md) guide to set up a ziti network first.
...
The Tunnelers need to connect to Public Edge-Router to pass traffic as depict in the [network diagram](#11-network-description). 
