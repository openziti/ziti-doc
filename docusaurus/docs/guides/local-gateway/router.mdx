...
title: Use a Router as a LAN Gateway
---

## 1.0 Introduction

This article is about using `ziti router run` in TPROXY mode as an infrastructure gateway providing access to services for a local subnet.
...
- An open-ziti network should be created already. If not, please follow this quickstart [Host OpenZiti Anywhere](/docs/learn/quickstarts/network/hosted/) guide to set up a ziti network first. **NOTE**, the router link listener port (**10080**) needs to be opened on the firewall if this is the only transit router you have on the network.
