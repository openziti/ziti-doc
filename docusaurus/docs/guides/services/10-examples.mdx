...
This guide provides both CLI and GUI (ZAC) instructions. To use ZAC, make sure ZAC is installed. If you have not installed ZAC, and would like to use it in this section, please follow the [ZAC Setup Guide](/guides/deployments/40-cloud.mdx#14-setup-ziti-administration-console-zac) before continuing.

## 3.2 Setup VMs for ziti network
### 3.2.1 Controller
#### 3.2.1.1 ZAC
Please make sure you can login to your network and see the welcome screen before continuing.
...
- Public Edge Router (**pub-er**) was setup in the [Router setup section](/guides/deployments/10-linux/20-router/60-cloud.mdx#23-create-and-setup-router-directly-on-router-vm), this router provides fabric and edge connection. It does not have tunneler functionality. Make sure you also modify the [Firewall](/guides/deployments/10-linux/20-router/60-cloud.mdx#29-firewall) for this router.
- Local Edge Router (**local-er**). Please follow the [Router guide](/guides/deployments/10-linux/20-router/60-cloud.mdx/) to setup a [Router with edge listener and tunneler](/guides/deployments/10-linux/20-router/60-cloud.mdx/#2343-create-the-router-with-edge-listener-and-tunneler). For this router, you will need to setup [Resolver](/guides/deployments/10-linux/20-router/60-cloud.mdx#26-fix-the-resolver), [Route Table](/guides/deployments/10-linux/20-router/60-cloud.mdx#27-route-table), [Source and Destination Check](/guides/deployments/10-linux/20-router/60-cloud.mdx#28-source-and-destination-check) and [Firewall](/guides/deployments/10-linux/20-router/60-cloud.mdx#29-firewall).
...
We need two tunnelers for our testing. Please follow **[Create a VM section](/guides/deployments/40-cloud.mdx#11-create-a-vm-to-be-used-as-the-controller)** of the Controller Guide to create two VMs running Ubuntu 22.04.
