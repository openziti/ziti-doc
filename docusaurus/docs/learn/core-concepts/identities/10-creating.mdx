...
established. Identities are integrally linked to the PKI configured in a given Ziti network and directly affect how
...
* One-time token (ott) identities using a third-party CA
* Third-party auto-enrolled identities

## Choosing an enrollment method

Choosing which type of enrollment your identity will use comes down to whether you are using a third-party CA or not. If the
network does not have a third-party certificate configured the only option is to use the one-time token (OTT) enrollment method.

If one or more third-party CA is installed you will need to understand the intention of each third-party certificate.
...
### Third-party CA: Overview

The controller is capable of using an existing PKI for authentication and authorization rather than the PKI
configured in the controller.  Certificates that are not controlled by the controller are referred to as "third
party". If you have an existing PKI setup you wish to reuse or if you are just interested in learning how
to use a third-party CA this section is for you.

:::info
Reusing a PKI is not a simple topic and managing and maintaining a PKI is out of the scope of this guide.
:::

A third-party CA will need to be created and the public certificate uploaded into the controller. After using an
existing PKI to reuse/generate a certificate, the controller will be able to create identities which will be expected to
present a certificate during the connection process that is valid per the provided certificate.

#### Adding a third-party CA to the Ziti controller
...
Third-party CA OTT enrollment is closely related to [OTT Enrollment](#one-time-token-ott). The main difference is the
utilization of a third-party CA certificate rather than the configured edge CA and PKI. In this method, the system
does not have access to the third-party CA private key and thus cannot provide CSR fulfillment capabilities. Instead it is
...
order to generate one-time token required and to create the jwt that can be delivered to enrolling devices. This means
...
<Tabs>
<TabItem label="New third-party CA identity via UI" value="tabid-new-identity-ui">
...
<TabItem label="New third-party identity via CLI" value="tabid-new-identity-cli">
