...
![Infinity datasource authentication configuration](/docusaurus/static/img/Infinity_noAuth.png)
...
![Infinity datasource network confioguration](/docusaurus/static/img/Infinity_CACert.png)
...
- Build the query as below, entering your Edge management API socket address 
  ...
  ![Variable query configuration](/docusaurus/static/img/bearer_token_variable.png)
...
- Open the URL options by selecting the Headers, Body, Request params button to the right of the URL and configure as below, entering the username and password from your identity.
 ...
 ![Variable query URL Options configuration](/docusaurus/static/img/bearer_token_variable_url_options.png) 
...
- In the Parsing options and Result fields, enter the below.  The Infinity datasource provides JSONPath functionality, which is used here to extract the API session token for use in the panel queries.
   ...
   ![Variable parsing options configuration](/docusaurus/static/img/bearer_token_variable_parsing.png)
...
- Add a new visualization to the dashboard, and configure the query as below.  
  ...
  ![Identities API Query configuration](/docusaurus/static/img/api_identities_query.png)

- Select the Headers, Request params button next to the URL field
- Enter the zt-session header as below.

![Query header configuration](/docusaurus/static/img/query_header_config.png)
...
![OpenZiti identity status](/docusaurus/static/img/identity_table.png)
