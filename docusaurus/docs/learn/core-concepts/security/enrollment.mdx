...
Enrollment is the process of some Edge client or edge router associating itself with an OpenZiti network. Client enrollment
results in the creation of an [Identity](./authentication/80-identities.md) or associating security credentials with a pre-provisioned [Identity](./authentication/80-identities.md).
OpenZiti Router enrollment is exclusively associating security credentials with a pre-provisioned Edge Router.

Edge clients "learn" the controller's address and port when they enroll, so it is necessary to re-enroll or replace the [Identity](./authentication/80-identities.md) if the controller's advertised address or port changes.
...
OTT Enrollment involves creating an [Identity](./authentication/80-identities.md) and then delivering the enrollment JWT to client software that can then
complete enrollment.
...
Alternatively, enrollments for an [Identity](./authentication/80-identities.md) can be reviewed at `/edge/management/v1/identities/<id>/enrollments` or
...
OpenZiti network without first creating an [Identity](./authentication/80-identities.md) or distributing a JWT enrollment token. Create a
[3rd Party CA](./authentication/30-third-party-cas.md) and ensure that `isAutoCaEnrollmentEnabled` is set to `true`.

The name of enrolling clients is controlled by the `identityNameFormat` of the [3rd Party CA](./authentication/30-third-party-cas.md).
The /format supports a number of replacement strings:

- `[caName]` - the OpenZiti `name` of the [3rd Party CA](./authentication/30-third-party-cas.md) that validates the enrolling certificate
- `[caId]` - the OpenZiti `id` of the [3rd Party CA](./authentication/30-third-party-cas.md) that validates the enrolling certificate
...
The `claimsProperty` that selects the external id of the enrolling [Identity](./authentication/80-identities.md) must be unique. Attempting to enroll
with further duplicate external ids will result in an error. If desired, remove the previously enrolled [Identity](./authentication/80-identities.md)
...
1. The client queries the public External JWT Signers list on the [Edge Client API](../../../reference/developer/api/01-edge-client-reference.mdx)
    ```text
