...
results in the creation of an [Identity](./authentication/60-identities.md) or associating security credentials with a pre-provisioned [Identity](./authentication/60-identities.md).
OpenZiti Router enrollment is exclusively associating security credentials with a pre-provisioned Edge Router.

Edge clients "learn" the controller's address and port when they enroll, so it is necessary to re-enroll or replace the [Identity](./authentication/60-identities.md) if the controller's advertised address or port changes.
...
- pre-provisioned - Identities are created before the client attempts to run and are provided with one-time-tokens to enroll
  - OTT, OTT CA
- post-provisioned - an [Identity](./authentication/60-identities.md) is created during enrollment
  - Auto CA

### OTT Enrollment

OTT Enrollment involves creating an [Identity](./authentication/60-identities.md) and then delivering the enrollment JWT to client software that can then
complete enrollment.
...
Alternatively, enrollments for an [Identity](./authentication/60-identities.md) can be reviewed at `/edge/management/v1/identities/<id>/enrollments` or
...
Ziti network without first creating an [Identity](./authentication/60-identities.md) or distributing a JWT enrollment token. Create a
...
- `[caName]` - the OpenZiti `name` of the [3rd Party CA](./authentication/10-third-party-cas.md) that validates the enrolling certificate
- `[caId]` - the OpenZiti `id` of the [3rd Party CA](./authentication/10-third-party-cas.md) that validates the enrolling certificate
...
preconfiguring an [Identity](./authentication/60-identities.md) for them.
...
The [External JWT Signers](./authentication/50-external-jwt-signers.mdx) options `enrollToCertEnabled` and
...
The `claimsProperty` that selects the external id of the enrolling [Identity](./authentication/60-identities.md) must be unique. Attempting to enroll
with further duplicate external ids will result in an error. If desired, remove the previously enrolled [Identity](./authentication/60-identities.md)
