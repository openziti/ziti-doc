...
Enrollment is the process of some Edge client or edge router associating itself with an OpenZiti network. Client
enrollment results in the creation of an [Identity](./authentication/80-identities.md) or associating security
credentials with a pre-provisioned [Identity](./authentication/80-identities.md).
OpenZiti Router enrollment is exclusively associating security credentials with a pre-provisioned Edge Router.

Edge clients "learn" the controller's address and port when they enroll, so it is necessary to re-enroll or replace
the [Identity](./authentication/80-identities.md) if the controller's advertised address or port changes.
...
- pre-provisioned - Identities are created before the client attempts to run and are provided with one-time-tokens
  to enroll
  - OTT, OTT CA
- post-provisioned - an [Identity](./authentication/80-identities.md) is created during enrollment
  - Auto CA

### OTT Enrollment

OTT Enrollment involves creating an [Identity](./authentication/80-identities.md) and then delivering the enrollment
JWT to client software that can then complete enrollment.
...
Alternatively, enrollments for an [Identity](./authentication/80-identities.md) can be reviewed at
`/edge/management/v1/identities/<id>/enrollments` or `/edge/management/v1/enrollments` or
...
[3rd Party CA](./authentication/30-third-party-cas.md). When creating an identity the `id` of the target
[3rd Party CA](./authentication/30-third-party-cas.md) is specified.
...
Auto CA enrollment allows a [3rd Party CA](./authentication/30-third-party-cas.md) to have clients enroll with a
OpenZiti network without first creating an [Identity](./authentication/80-identities.md) or distributing a JWT
enrollment token. Create a
[3rd Party CA](./authentication/30-third-party-cas.md) and ensure that `isAutoCaEnrollmentEnabled` is set to `true`.

The name of enrolling clients is controlled by the `identityNameFormat` of the
[3rd Party CA](./authentication/30-third-party-cas.md).
The /format supports a number of replacement strings:

- `[caName]` - the OpenZiti `name` of the [3rd Party CA](./authentication/30-third-party-cas.md) that validates the
  enrolling certificate
- `[caId]` - the OpenZiti `id` of the [3rd Party CA](./authentication/30-third-party-cas.md) that validates the
  enrolling certificate
...
[External JWT Signers](./authentication/70-external-jwt-signers.mdx) represent an external IdP that can sign JWTs
as proof of identity (including OAuth/OIDC servers). Auto Ext-JWT-Signer enrollment allows a client with a JWT from a
configured [External JWT Signers](./authentication/70-external-jwt-signers.mdx) to enroll without an administrator
preconfiguring an [Identity](./authentication/80-identities.md) for them.
...
- the External JWT Signer has a `claimsProperty` set that maps to an id value to use as `externalId` of enrolling 
    Identities.
- either the `enrollToCertEnabled` or `enrollToTokenEnabled` are set to `true`


The [External JWT Signers](./authentication/70-external-jwt-signers.mdx) options `enrollToCertEnabled` and
...
The `claimsProperty` that selects the external id of the enrolling [Identity](./authentication/80-identities.md)
must be unique. Attempting to enroll with further duplicate external ids will result in an error. If desired, remove
the previously enrolled [Identity](./authentication/80-identities.md) to allow re-enrollment.
...
1. The client queries the public External JWT Signers list on the
...
Certificate enrollment requires the creation of a
[Certificate Signing Request (CSR)](https://en.wikipedia.org/wiki/Certificate_signing_request). The CSR can contain
any values desired. However, they will be ignored by the controller. The only requirement is that a properly
configured key set was used to create and sign the CSR.
...
OpenZiti. If the client's certificate was issued by a [3rd Party CA](./authentication/30-third-party-cas.md), the
client certificate renewal must be handled outside of OpenZiti.
