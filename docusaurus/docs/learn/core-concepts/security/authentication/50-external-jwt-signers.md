...
External JWT Signers can be used to enroll identities that are no pre-created by administrators. Enrollment is 
controlled by the `enrollToCertEnabled` and `enrollToTokenEnabled` fields on `enabled` External JWT Signers. See the 
[Enrollment - Auto External JWT Signer Enrollment](../enrollment.md#auto-external-jwt-signer-enrollment) 
