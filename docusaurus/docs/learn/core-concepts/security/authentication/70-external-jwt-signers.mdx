...
a JWT as a primary authentication mechanism to obtain an
[API Session](/learn/core-concepts/security/sessions.md#api-session).
Additionally, the JWT can be required on all REST API calls if desired by using an Authentication Policy that requires
it as a secondary factor.

JWT is described in [RFC 7519](https://datatracker.ietf.org/doc/html/rfc7519) and on
[Wikipedia](https://en.wikipedia.org/wiki/JSON_Web_Token).
X509 PKI is described in [RFC 5280](https://datatracker.ietf.org/doc/html/rfc5280) and on
[Wikipedia](https://en.wikipedia.org/wiki/X.509)

## Usage

External JWT Signers are used in conjunction with Authentication Policies to allow [Identities](./80-identities.md)
to authenticate using a JWT. External JWT Signers can be configured to match an [Identity](./80-identities.md)
through the `claimsProperty` and `useExternalId` fields. `claimsProperty` can change which field in the JWT is
matched to the `id` or `externalId` on an [Identity](./80-identities.md). `useExternalId` when true will match the
`claimsProperty` to an Identity's `externalId` field. If false, it will match to the `id` field.

_Note: `externalId` values on [Identities](./80-identities.md) are enforced to be unique and are matched
case-sensitively. The value from the JWT claim must match the identity's `externalId` exactly._
...
External JWT Signers can be used to enroll [Identities](./80-identities.md) that are not pre-created by
...
- the `sub` or field defined by `claimsProperty` must match the `id` field on an [Identity](./80-identities.md)
  (or `externalId` if `useExternalId` is true)
...
[Edge Client API](../../../../reference/developer/api/01-edge-client-reference.mdx). The response contains only the
