...
External JWT Signers are used in conjunction with Authentication Policies to allow [Identities](./60-identities.md) to authenticate using a
JWT. External JWT Signers can be configured to match an [Identity](./60-identities.md) through the `claimsProperty` and `useExternalId`
fields. `claimsProperty` can change which field in the JWT is matched to the `id` or `externalId` on an [Identity](./60-identities.md).
`useExternalId` when true will match the `claimsProperty` to an Identity's `externalId` field. If false, it will match
to the `id` field.

_Note: `externalId` values on [Identities](./60-identities.md) are enforced to be unique._

`claimsProperty` can contain JWT standard claims or private claims. An example usage would be an `email` JWT private
claim and OpenZiti [Identities](./60-identities.md) with `externalId` set to email addresses.

### Enrollment

External JWT Signers can be used to enroll [Identities](./60-identities.md) that are not pre-created by administrators. Enrollment is 
...
- the `sub` or field defined by `claimsProperty` must match the `id` field on an [Identity](./60-identities.md) (or `externalId` if `useExternalId` is true)
