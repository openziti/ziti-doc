...
[Authentication Policies](50-authentication-policies.md) and [Posture Checks](/learn/core-concepts/security/authorization/posture-checks/00-overview.md).
...
When enforced through the [MFA Posture Check](/learn/core-concepts/security/authorization/posture-checks/00-overview.md), clients still must become fully
authenticated according to their [Authentication Policy](50-authentication-policies.md), however, access to services is determined by their policy and
...
Enrollment associates a TOTP secret with an [Identity](80-identities.md). The controller generates a secret and 20 single-use recovery
...
- **During an OIDC authentication flow** - if the [Authentication Policy](50-authentication-policies.md) requires TOTP
  but the [Identity](80-identities.md) is not yet enrolled, enrollment can be completed mid-flow before the authorization
  code is issued. See [OIDC Authentication - TOTP Enrollment](10-oidc.md#totp-enrollment-during-an-oidc-flow) for the
