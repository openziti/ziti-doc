...
quickstart will install your Ziti network's PKI and configuration files in `${HOME}/.ziti/quickstart/$(hostname -s)`. You may choose a different location by defining `ZITI_HOME5=/custom/path/to/quickstart`. If you do customize `ZITI_HOME6` then you should also make this assignment in your shell RC, e.g., `~/.bashrc` for future convenience.
...
```text
sudo cp "${ZITI_HOME1}/${ZITI_CTRL_NAME}.service" /etc/systemd/system/ziti-controller.service
sudo cp "${ZITI_HOME2}/${ZITI_ROUTER_NAME}.service" /etc/systemd/system/ziti-router.service
...
If you log out and log back in again you can source the *.env file located in `ZITI_HOME3`.
...
- Help
  ...
  - [Reset the Quickstart](./help/reset-quickstart)
